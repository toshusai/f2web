...
import { WebNode, varIdToVariableName, TextPropsBridge } from ".";
...
function changeToProps(componentSet: ComponentSetNode) {
  const obj = {};
...
export function figmaNode2WebNode(
  ...
  if (node.type === "INSTANCE" && options?.useInstance) {
    ...
    return {
      ...
      props: {
        name: node.name,
        props: changeToProps(node.parent as ComponentSetNode),
      },
    };
  }
  if (!node.visible) {
  ...
  if (node.type === "TEXT") {
      ...
      fillColor: (() => {
        ...
        if (fill.type === "SOLID") {
          if (fill.boundVariables) {
            if (!fill.boundVariables.color) return undefined;
            return fill.boundVariables.color.id;
          }
    ...
    return {
      children: node.children
        ...
        ?.map((child) =>
          figmaNode2WebNode(child as FrameNode | InstanceNode, root, options)
        )
